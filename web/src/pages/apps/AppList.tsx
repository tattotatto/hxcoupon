...
export default function AppList() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await storeApi.listApps(filters);
  ...
  const handleSave = async () => {
    ...
    try {
      if (modal.editing) {
        await storeApi.updateApp(modal.editing.id, values);
        message.success('更新成功');
      } else {
        await storeApi.createApp(values);
  ...
  const handleCredentials = async (id: number) => {
    try {
      const res = await storeApi.generateAppCredentials(id);
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (s: number) => (
        <Tag color={s === 1 ? 'success' : 'error'}>{s === 1 ? '启用' : '禁用'}</Tag>
      ),
    },
